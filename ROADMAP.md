...
- **Minimal HMR candidate seam — source shipped disabled 2026-09-04.** `@lunarmoon26/dal-hmr-candidate` stages fixed plugin files, verifies the exact candidate digest at application, consumes successful DSH HMR reloads, marks generation-spanning sessions ineligible through `dal-run-record`, and restores the prior source snapshot. A pinned real DSH Loader/HMR composition probe, shared-profile mounting, and production deployment remain unproved and separately approval-gated.
...
- Split the plugin system into run mode (the agent operates and actions/outcomes are recorded; no improvement code in the hot path) and improvement mode (dsh becomes an isolated workbench). **Shipped v0 source**: `@lunarmoon26/dal-run-record` projects privacy-safe run records and candidate-generation eligibility; `@lunarmoon26/dal-improve-tools` exposes deterministic read/evaluate tools; `@lunarmoon26/dal-hmr-candidate` stages, approval-verifies, HMR-admits, and rolls back one bounded plugin candidate. The workbench rows are disabled by default and profile mounting remains an approval-gated `install_or_mount_plugin` operation.
...
- Deploy accepted profile changes from workspace-owned plugin packages: the HMR workbench now covers temporary isolated evaluation and source rollback, but production installation remains a human, approval-gated derived generation. Keep the previous installed generation bootable and switch only after acceptance; track upstream for project-level `.dsh` profiles.
...
- Complete the remaining audited hardening list: rubric isolation with drift re-marking, code-owned scoring over LLM leaf judgments, durable crash recovery for HMR rollback, and a production A/B generation switch with a bootable previous slot. One-shot sealed reveal, nonce-bound admission receipts, exactly-once effect sagas, and in-process HMR source rollback already ship. For any future LLM-judge rubric tier, the UniPat Echo rubric-search pattern applies with grader verdicts as ground truth: maximize rubric-vs-grader ranking agreement on held-out cases, search the rubric (never hand-design it), and freeze the search objective.
