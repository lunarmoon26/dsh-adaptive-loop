...
- Priority 1: safe candidate staging is implemented under `chg-dal-evolution-staging-integrity-20260905`; see [the staging contract](docs/optimizer-staging.md). `chg-dal-proposer-receipt-boundary-20260906` adds a [payload-only proposer](docs/proposer-request.md) instead of executing an unconstrained DSH profile. Approval binds the complete versioned HTTPS request. Agentic proposer confinement and other providers remain future extensions. Full increment closure requires contract/capsule review and the full local gate; no live model execution is claimed.
- Priority 2: `chg-dal-proposer-receipt-boundary-20260906` implements candidate artifact/task/verdict binding, evidence replay, and execution deduplication for [branch scoring](docs/branch-evidence.md). Independent execution attestation, complete comparison-context binding, and integration of real candidate runs remain future work. Historical incomplete evidence is not rewritten into trusted proof.
...
- **Run-to-run observation foundation — shipped 2026-09-02.** `controller-policy.v1`, `controller-state.v1`, and `dal control estimate` derive deterministic, immutable Wilson-interval estimates from one compatible task batch and pinned harness generation. The optional run-mode `controllerObservation` configuration enrolls closed terminal session records into a batch; estimation additionally requires the runtime attestation gate below. Checkpoints, incomplete sessions, context contradictions, and default unconfigured records remain unbatched. Mixed context/generation evidence fails closed; insufficient evidence is explicit and non-authorizing. Business/check outcome joining remains future evaluator work. The supervisor state is separate from proposal lifecycle and performs no proposer, model, budget, application, promotion, or rollback action.
...
- Recognize the current deployment reality: dal is repository-scoped, so a fresh workspace has no dal skill, instructions, CLI, store, or mounted plugin. Available tiers are repo-scoped vendoring, the approval-gated user-global skill plus fixed `AGENTS.md`, and the approval-gated Cordis bundle.
- **HMR candidate staging quarantined 2026-09-05.** `@lunarmoon26/dal-hmr-candidate` retains fixed inactive staging and digest inspection, but application now fails closed before approval verification or live writes. Source inspection and executable probes showed that current `hmr/reload` precedes Fiber readiness and that sequential multi-file publication can load a hybrid closure. Implement trusted lifecycle observation, immutable imported-closure identity, awaited readiness, failed-start rollback, and durable recovery before reconsidering admission.
...
- Split the plugin system into run mode (the agent operates and actions/outcomes are recorded; no improvement code in the hot path) and improvement mode (dsh becomes an isolated workbench). **Shipped v0 source**: `@lunarmoon26/dal-run-record` projects privacy-safe run records; `@lunarmoon26/dal-improve-tools` exposes deterministic read/evaluate tools; `@lunarmoon26/dal-hmr-candidate` stages fixed files but rejects application. The workbench rows are disabled by default and profile mounting remains an approval-gated `install_or_mount_plugin` operation.
...
- Complete the remaining audited hardening list: rubric isolation with drift re-marking, code-owned scoring over LLM leaf judgments, trusted HMR lifecycle/readiness evidence, durable transition recovery, and a production A/B generation switch with a bootable previous slot. One-shot sealed reveal, nonce-bound admission receipts, and exactly-once effect sagas already ship; in-process HMR admission and rollback do not. For any future LLM-judge rubric tier, the UniPat Echo rubric-search pattern applies with grader verdicts as ground truth: maximize rubric-vs-grader ranking agreement on held-out cases, search the rubric (never hand-design it), and freeze the search objective.
