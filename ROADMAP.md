...
- **Split harness outcome from business outcome.** A dsh run that completed but graded as a business failure must record `outcome: succeeded` (harness) with `business_outcome.status: failed`, reserving `failed` for crashes, timeouts, cancelled runs, corrupted logs, sandbox failures, and composition mount failures. Clustering then keys on business failures separately instead of conflating wrong decisions with runtime crashes. Requires a run-record semantics update plus a grader-version bump and rebaseline.
- **Require refusal trajectories.** Refusal tasks must pass only when the state is unchanged AND a `refuse_request` effect exists with the correct target and a policy-consistent reason AND no forbidden effect exists. Needs task-level required/forbidden effect metadata (evaluator-only, never agent-visible) and a `GRADER_VERSION` bump.
- **Physical oracle isolation.** Stop bind-mounting the whole repository into the candidate container: stage a minimal read-only workspace (agent-visible task, policy, candidate skill, tools plugin) and move the mock service into a separate principal with a typed endpoint, so `goal_state`, grader source, effect logs, and receipts are unreachable at runtime. The three-container topology (candidate / service / grader) is the target.
- **Crash-safe service state.** Replace persist-state-then-append-effect with an append-only effect journal as the source of truth (state derived by replay) or a single-transaction SQLite store, closing the crash window between state write and effect append and the corrupt-state-loads-as-empty failure.
- **Composition-patch text in the approval manifest.** The manifest currently binds the patch's inputs plus the executed tools bytes; also bind the rendered patch text itself once per-batch state roots stop churning the digest.
- **G2 harness-code adaptation.** After the skill-only G0/G1 experiment: a candidate Cordis plugin (unknown-effect guard) that locks same-key retries behind a status check, installed as its own generation with rollback.
