...
describe("G0 skill gap guard", () => {
  it("keeps the G1 candidate's unknown-outcome guidance on the skill surface", async () => {
    ...
    expect(skill).toContain("get_effect_status");
    expect(skill).toContain("unknown");
  });

  it("keeps the G0 baseline skill free of the unknown-outcome guidance (the deliberate gap)", async () => {
    const { readFile } = await import("node:fs/promises");
    const { resolve } = await import("node:path");
    const baseline = await readFile(
      resolve(import.meta.dirname, "..", "benchmarks", "tau-style-workflow", "dal", "fixtures", "skill-g0-v1.md"),
      "utf8",
    );
    expect(baseline).not.toContain("get_effect_status");
    expect(baseline).not.toContain("unknown");
  });
});
