...
Batch enrollment does not bypass runtime attestation. New estimates require a 1.1.0 controller policy and stable, policy-qualified repository evidence from the launcher-owned binding. If a binding exists, its harness, model-patch, and harness-pin identities remain authoritative; contradictions with configured pins or a changed transition sequence leave the record unbatched. Configured pins alone never manufacture runtime evidence. HMR candidate admission remains quarantined.
