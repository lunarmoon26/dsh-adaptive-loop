...
15. Dependency and command inspection confirms no model judge, generic network client, policy daemon, plugin installer or mounter, optimizer runner, or candidate applier ships; the fixed user-global installer and purpose-specific proposer/e2e executors independently require exact approvals at their operation boundaries.
...
22. Given two terminal run-mode sessions with one explicit compatible `controllerObservation` configuration, their final records validate and feed `dal control estimate` directly; checkpoint, incomplete, contradictory, and unconfigured records do not join the batch; raw session content remains absent.
...
- Not proved in v0: dsh lifecycle-level enforcement, optimizer quality, controller improvement or stability, sandbox isolation, multi-machine durability, or model compliance with project instructions.
