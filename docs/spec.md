...
`dal propose prepare --clusters <dir> --provider <route> --model <id> --budget <file> [--runs <dir>] --output <request-file>` builds a sanitized payload and strict v2 HTTPS request. Supported exact routes are OpenAI `gpt-5.6-terra`, Anthropic `claude-sonnet-5`, and explicit-model `deepseek-official` (the legacy default provider). `request_digest` binds the fixed provider endpoint, model, instructions, complete body, protocol headers, transport limits and budget allocation; `payload_digest` remains source provenance. `dal propose run` requires exact unexpired `send_data_externally` approval and durable per-provider budget reservation before credential access or sending. Repeated request digests, exhausted allocations and existing draft destinations fail before sending; failures retain reservations without refund or automatic retry. It loads no workspace, DSH profile, tools, subprocess, or `.env` file. Docker execution remains rejected. Bounded native provider transports reject redirects, tools and incomplete replies; OpenAI reasoning items are discarded while a single completed assistant answer is required. Replies and drafts are schema-validated and privacy-scanned before persistence. V1 is retained for historical records only; new requests require fresh v2 approval. See [proposer request contract](proposer-request.md). The automatic stage generates a proposal draft, not an applied artifact. Reservation accounting is not provider billing or e2e-wide dollar enforcement.
