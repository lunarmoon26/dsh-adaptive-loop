...
`dal propose prepare --clusters <dir> --model <id> [--runs <dir>] --output <request-file>` builds a sanitized payload and a strict versioned HTTPS request. The printed `request_digest` binds the fixed DeepSeek endpoint, explicit model, instructions, complete body, and transport limits; `payload_digest` remains separate source provenance. `dal propose run` recomputes this request and requires exact unexpired `send_data_externally` approval against its request digest before credential access or sending. It loads no workspace, DSH profile, tools, subprocess, or `.env` file. Docker execution is rejected. The fixed text-only transport has bounded request/response bytes and timeout, no redirects/retries, and rejects tool-call replies. Replies and drafts are schema-validated and privacy-scanned before persistence; drafts carry `provenance.request_sha256`. Old payload approvals require preparation and review again. See [proposer request contract](proposer-request.md); proposal generation does not evaluate or activate an artifact.
