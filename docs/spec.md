...
23. Given a configured workspace-owned plugin entry, approval verification runs only from unchanged startup-pinned launcher files outside that worktree, a successful matching HMR reload admits its bounded source digest, failed/unrelated/mismatched reloads abort admission, checkpoints and sessions spanning a later reload are ineligible, and rejection restores and reactivates the exact prior bytes without modifying dsh core or a shared profile.
