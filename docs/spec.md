...
DSH Adaptive Loop (`dal`) provides a local, human-governed improvement loop for those closed-loop workflow classes around DeepSeek Harness (`dsh`). Version 0 captures structured end-of-task feedback, validates and aggregates it, stages improvement proposals, and records human decisions. It does not modify itself, dsh, shared harness configuration, or installed plugins.
...
- Autonomous self-modification, automatic prompt mutation, or automatic Cordis composition changes.
...
Version 0 implements the deterministic core of the improvement loop and keeps every model-dependent tier future-only. `dal run ingest` validates and immutably stores one run record: task and change identity, outcome, structured failure facts, the pinned evaluation context (task set, environment snapshot, tool versions, model, prompt/harness digests, grader version, seeds, context policy digest, inference parameters), workspace artifact digests, usage metrics, evidence references, and privacy metadata. `dal cluster run` computes canonical failure fingerprints from the structured failure facts, groups failed runs by exact signature, and exclusively publishes immutable cluster records (Tier 0/1); successful runs are never clustered. Cluster identity binds the failure fingerprint to the run batch: records carry an additive `batch_id`, the cluster id folds the batch in, and `dal cluster run --batch <id>` filters to one batch — so iterative loops re-cluster a growing store cleanly instead of colliding (`CLUSTER_ID_CONFLICT`). The clustering command runs no model, embedding, classifier, or optimizer; Tier 2/3 clustering, proposer integration (GEPA/SkillOpt), and the run/improvement dsh plugin modes remain future work in the roadmap. The governing rule is enforced at the proposal boundary: the agent may propose changes to the editable surfaces, but it cannot control the evaluator, the sealed holdout, its permissions, the promotion policy, the audit log, or the rollback mechanism.
...
- **Run mode** (`@lunarmoon26/dal-run-record`): a persistence-style plugin that subscribes to the dsh `session/event` firehose, buffers fire-and-forget (observer failures are contained and never fail an append), and drains on `session/flush` plus session disposal into workspace-local run records under `.dal/runs/`. Records carry only privacy-safe projections: session identity, outcome from turn-end reasons, failure facts from tool-result errors (names and codes only), per-tool call counts, summed token usage, the provider/model route, `sha256` digests of the system prompt and the workspace policy file, and the environment snapshot. Raw prompt text, message content, tool arguments, and tool results are never stored — only their digests or counts.
...
15. Dependency and command inspection confirms no model judge, network client, policy daemon, plugin installer, optimizer runner, shared-config writer, or candidate applier ships.
...
- Not proved in v0: dsh lifecycle-level enforcement, optimizer quality, sandbox isolation, multi-machine durability, or model compliance with project instructions.
