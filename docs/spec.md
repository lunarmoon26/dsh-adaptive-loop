...
DSH Adaptive Loop (`dal`) provides a local, human-governed improvement loop for those closed-loop workflow classes around DeepSeek Harness (`dsh`). Version 0 captures structured run evidence, stages workspace-owned Cordis plugin candidates and improvement proposals, and records human decisions. HMR candidate application is quarantined because current dsh module HMR does not attest the imported closure or await replacement-Fiber readiness. Candidate authoring happens in an isolated worktree and never modifies dsh core, shared harness configuration, or an installed production plugin.
...
- A workbench operator can stage and digest one bounded plugin candidate without changing loaded files; runtime evaluation remains unavailable until the readiness contract is implemented.
...
- Preserve Cordis effect disposal and dsh HMR as version-pinned evaluation subjects without treating an HMR event or post-event disk digest as runtime identity.
...
- **Candidate generation:** reserved runtime identity for one configured workspace-owned plugin closure after authoritative import and readiness evidence; current HMR staging produces no admitted candidate generation.
...
An approved human decision is required before an operation changes shared harness configuration, installs a plugin, sends data externally, or applies an optimization candidate. Decisions are action- and scope-specific, expire, and cannot authorize another action by implication. v0 contains no general implementation of those actions. The HMR workbench stages inactive copies but rejects application with `CANDIDATE_ADMISSION_QUARANTINED`; no current DAL operation applies a plugin candidate to loaded source.
...
`dal propose prepare --clusters <dir> --provider <route> --model <id> --budget <file> [--runs <dir>] --output <request-file>` builds a sanitized payload and a provider-versioned HTTPS request. Supported exact routes are OpenAI `gpt-5.6-terra`, Anthropic `claude-sonnet-5`, and explicit-model `deepseek-official` (the legacy default provider). OpenAI and DeepSeek requests remain byte-for-byte v2; new Anthropic Sonnet 5 requests use v3 with exact `thinking: {type: "disabled"}`. V1/v2 historical requests remain unchanged, and an old Anthropic approval cannot authorize v3. Native DSH profile `reasoning: off` retains reasoning capability (there is no `reasoningEfforts: false`); the gateway requires `disabled`, with no adaptive, enabled, or replay modes currently. `request_digest` binds the fixed provider endpoint, model, instructions, complete body, protocol headers, transport limits and budget allocation; `payload_digest` remains source provenance. `dal propose run` requires exact unexpired `send_data_externally` approval and durable per-provider budget reservation before credential access or sending. Repeated request digests, exhausted allocations and existing draft destinations fail before sending; failures retain reservations without refund or automatic retry. It loads no workspace, DSH profile, tools, subprocess, or `.env` file. Docker execution remains rejected. Bounded native provider transports reject redirects, tools and incomplete replies; OpenAI reasoning items are discarded while a single completed assistant answer is required. Replies and drafts are schema-validated and privacy-scanned before persistence. Payload-only approvals do not authorize new sends. See [proposer request contract](proposer-request.md) and [Anthropic disabled reasoning](anthropic-off.md). The automatic stage generates a proposal draft, not an applied artifact. Reservation accounting is not provider billing or e2e-wide dollar enforcement.

### DAL-016 — Bounded search branches

Drafts become branches in an archive. `dal branch record --branch <id> --draft <file> [--candidate <artifact-file>] [--parent <branch-id>]` records parent linkage and optionally the actual candidate artifact reference/digest independently of the draft. Receipt-backed scoring requires that artifact binding, the exact full task-file digest (`task_sha256`), state/effect digests, and a non-null exact recomputed verdict digest. `dal branch evaluate` without a receipt remains diagnostic and never earns visits. Versioned proofs are replayed from their referenced bytes by `dal branch stats`; legacy provenance booleans are not sufficient. Stable session/task execution identities make retries idempotent and conflicting receipt reuse fails closed. Malformed or drifted evidence cannot yield partial statistics. See [branch evidence](branch-evidence.md) for field semantics and migration limits. UCB1 selection is unchanged and remains exploratory, not activation authority. Receipt integrity alone is not independent proof that a candidate actually ran.
...
- **Optional terminal observations:** explicit `controllerObservation` configuration fixes a batch and its task-set and generation/context pins. Only final records after a closed turn with a recognized terminal reason receive that batch ID; checkpoints, incomplete sessions, unsupported reasons, observed-context contradictions, and unconfigured records remain unbatched. Missing or unsupported terminal reasons are recorded conservatively as aborted. Launcher-owned generation pins take precedence; contradictions or unstable bindings prevent batch enrollment. Configured provenance does not replace DAL-024 attestation, grant HMR admission, or fabricate business outcomes or deterministic checks.
...
- **HMR staging helper** (`@lunarmoon26/dal-hmr-candidate`): disabled and quarantined. It can copy configured files into inactive staging and report digests, but `dal_candidate_apply` always fails before approval verification or a live-file write. It publishes no admitted generation.

The bundle ships with the recorder row enabled and the improvement-tools and HMR-candidate rows disabled; a workbench profile may enable deterministic improvement tools, while enabling the HMR row still cannot apply a candidate. Mounting any mode into a profile is an approval-gated `install_or_mount_plugin` operation (bundle sources stay workspace-VCS-owned and digest-pinned); the CLI needs no dsh profile to work. Deeper dsh integration (trusted HMR observability/readiness, shared run-store location across machines, the separate-principal holdout service, MCTS rollouts) remains roadmap work.
...
The verifier and benchmark retain their pinned container paths. `dal verify run --runner docker` revalidates its guardrail decision inside the `deploy/docker/` image and confines through the sandbox chain with network denied; the selected image, flags, and environment names remain policy-owned. The proposer no longer uses this workspace-mounting path: `dal propose run --runner docker` fails closed, and only the approved payload-only HTTPS request in DAL-015 is supported. See `deploy/docker/README.md`. Container availability and sandbox proof remain platform-specific; no host profile is changed.
...
Explicit rehearsal mode uses native deterministic protocol fixtures with no provider key or outbound gateway network. It records successful tool/finish protocol evidence separately from business outcomes. Rehearsal records, receipts and summaries are confined to `.dal/check/rehearsal-runs/<campaign>`; ordinary proposal preparation rejects recognizable rehearsal sources, including legacy records. Daemon isolation snapshots are compared before and after execution and owned resources are cleaned without deleting the durable budget ledger. Execution receipts still bind state/effects, verdict, candidate/model/generation and manifest identities; comparisons reject missing, repeated or drifted evidence and recompute metrics. These integrity and protocol checks do not establish model capability or a measured adaptation gain. `g2` remains rejected and HMR candidate application stays quarantined.

The repaired text-budgeted OpenAI rollout uses an image-bound caller-side
Responses projection: explicit reasoning effort `none`, removal only of
empty-visible opaque reasoning history, and preservation of complete tool calls,
outputs and visible messages. It removes optional native function item IDs when
paired reasoning is removed, retaining `call_id` linkage. Nonempty visible
reasoning fails closed. The gateway continues to reject encrypted/reference-only
reasoning; the profile and source/image changes require fresh paid approval.
Gateway failure receipts now retain bounded closed-vocabulary stage/guard codes,
observed HTTP status and allowlisted provider error identifiers, never raw
messages or payloads. Missing launcher credentials fail after exact authorization
but before claiming an attempt. See [text replay repair](openai-text-replay.md).
...
### DAL-025 — Quarantined HMR plugin candidate staging

The workbench package configures one loaded plugin entry, a bounded list of existing source files, one fixed staging directory, and the DSH version/profile under an isolated linked git worktree. Legacy approval-file, approval-scope, launcher, and timeout fields remain accepted for configuration compatibility but are ignored while admission is quarantined. `dal_candidate_prepare` copies the current files into staging; the agent edits only those inactive copies. `dal_candidate_status` reports the source and staged digests plus the base git tree. `dal_candidate_apply` always fails with `CANDIDATE_ADMISSION_QUARANTINED` before approval verification or any loaded-file mutation. An exact human decision cannot bypass this code-owned safety block.
...
15. Dependency and command inspection confirms no model judge, generic network client, policy daemon, plugin installer or mounter, optimizer runner, or candidate applier ships; the fixed user-global installer and purpose-specific proposer/e2e executors independently require exact approvals at their operation boundaries, while HMR application fails closed in code.
...
17. The mode bundle ships recorder-on/tools-off/HMR-off; run-mode records validate against the run-record schema and contain no raw prompt, message, argument, or result content; deterministic workbench tools expose no approval-gated operation, and the quarantined HMR apply tool mutates nothing.
...
- Not proved in v0: DSH production of authoritative runtime-generation manifests/evidence, HMR imported-closure identity or activation readiness, dsh lifecycle-level enforcement, optimizer quality, controller improvement or stability, production-profile promotion, sandbox isolation, multi-machine durability, or model compliance with project instructions.
