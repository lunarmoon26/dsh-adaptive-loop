...
| Self-improvement reward hacking | A proposal optimizes the measured score while degrading safety or hidden behavior | Objectives, held-out fixtures, guardrail metrics, regression cases, and application authority are separate. An optimizer cannot edit its evaluator, policy, approval, scorecard, fixed HMR target list, or promotion path. Candidate application requires an exact digest-bound decision. | Finite suites can be gamed. Maintainers own hidden-case rotation, qualitative review, and later production monitoring. |
