...
### Upstream audit: sortit, SaaS-Bench, EvoCodeBench
...
| --- | --- | --- | --- |
| sortit, `getstackit/sortit` (issue tracker, not a code improver) | Continuous tag relevance scores (0..1) with **negation scores** and per-tag provenance/verifier metadata; **retrieval-first candidate taxonomy** (embed → cosine-retrieve nearby tag embeddings → score only the shortlist); deterministic **specificity** = embedding-neighborhood cosine distances converted to catalog percentile ranks; display ranking blends relevance + specificity; factor-model 2D map for clustering (`docs/scoring-search-map.md`, `docs/architecture.md`). | Proposer inspiration (the user's target): (a) bounded candidate shortlisting before scoring — the shape of our cluster→prepare→propose pipeline; (b) an explicit negation signal per hypothesis (a proposal can assert "this does NOT apply" with provenance) — richer than our single-fingerprint buckets; (c) deterministic specificity ranks as ranking tie-breakers; (d) per-tag provenance ("which candidate source selected it") maps onto branch/cluster provenance. | **Adapt the scoring ideas** — soft relevance + negation + specificity as a future proposer ranking layer over our deterministic cluster fingerprints; nothing to adopt as a framework. |
| SaaS-Bench, `UniPat-AI/SaaS-Bench` | Verifier-as-ground-truth protocol: each task's `verify.py` inspects the running app (SQL via docker exec, HTTP probes) and prints `[PASS\|FAIL] (Npt) label` lines plus a `SCORE:` line; the harness parses them and normalizes `score = earned/total` (`saas_bench/verify_runner.py:240-303,308-363`; `docs/verify_protocol.md:40-69`). **Weighted partial credit** (per-check weights, e.g. 4/3/3/2/8 in `tasks/multi-m/Agriculture/agriculture_003/verify.py:223-344`) and **gated chained checks** (later checks short-circuit with `gated: <reason>` when upstream state is invalid, `:232-235,263-269,282-289,315-317`). Variance-aware aggregation: `pass_at_k`, `best_score`, `mean_score`, `checkpoint_score` (union across runs) (`saas_bench/reporting.py:107-169`), plus a self-report vs verifier calibration matrix (`:395-433`). Agent LLM judge explicitly disabled (`saas_bench/agent.py:698`, `judge=None`); no seeds — only the verifier is deterministic. | The tau-style grader currently emits binary per-check verdicts; SaaS-Bench shows a strictly richer deterministic contract: weighted partial credit, gated chaining to avoid double-penalizing, and pass@k unions. The calibration matrix answers "is the agent's own success signal trustworthy" — the same question our run-record outcome mapping faces. | **Adapt the verifier protocol** — add optional per-check weights + `gated` short-circuits to the workflow-grader contract, and adopt checkpoint-union metrics for multi-seed e2e; keep our digest-pinned checks and frozen evaluator. |
| EvoCodeBench, `UniPat-AI/EvoCodeBench` | Deterministic per-step binary rewards read from Harbor trial dirs (`steps/<name>/verifier/reward.txt`), aggregated as mean per-step reward, MT@k, final-step completion, and all-steps-passed rate (`evaluation/compute_metrics.py`); one attempt per task, full re-run discipline. **Evaluation-integrity incident (2026-06-20, CHANGELOG)**: Harbor's shared multi-step verifier mode left the grader (`/tests/test.sh`) and prior verifier outputs (`/logs/verifier/reward.txt`) readable by the agent in its own step, so agents could read the grader mid-task; fixed by clearing grader directories before each agent phase, with all prior results superseded by a clean re-run. | The incident is a real-world, citable precedent for dal's evaluator-isolation anchor and sealed holdout: grader artifacts must never sit in agent-readable paths, and contamination means superseding numbers, not patching them. | **Cite as anchor evidence** — add the grader-artifact isolation checklist (verifier files and logs outside agent-visible/agent-writable paths) to the evaluation documentation, and keep quarantine-plus-rerun as the contamination response. |

### Borrow list ranked (file:line anchors)

1. Weighted + gated verifier protocol — `SaaS-Bench/saas_bench/verify_runner.py:240-303`, `tasks/multi-m/Agriculture/agriculture_003/verify.py:223-344`.
2. Deterministic verifier interface contract (stderr protocol, exit-code-agnostic, timeout) — `SaaS-Bench/docs/verify_protocol.md:40-69`.
3. Evaluator-artifact isolation checklist — `EvoCodeBench/CHANGELOG.md` (2026-06-20 entry; Harbor issues #1960/#1961).
4. Soft relevance + negation + specificity ranking for the proposer — `sortit/docs/scoring-search-map.md` (Tag Relevance, Tag Candidate Selection, Tag Specificity sections).
