...
export function createWorkflowServiceServer(
  env: ServiceEnvironment,
  options: { evaluatorToken: string },
): ReturnType<typeof createServer> {
  if (options.evaluatorToken.length < 32) throw new Error("evaluator token must contain at least 32 characters");
...
function parseProfile<T extends string>(raw: string | undefined, allowed: readonly T[]): Partial<Record<T, EffectOutcome>> {
  if (raw === undefined || raw === "") return {};
  const parsed = record(JSON.parse(raw) as unknown);
  const result: Partial<Record<T, EffectOutcome>> = {};
  for (const [key, value] of Object.entries(parsed)) {
    if (!allowed.includes(key as T) || !["success", "definite_failure", "unknown"].includes(String(value))) {
      throw new Error(`invalid service profile entry ${key}`);
    }
    result[key as T] = value as EffectOutcome;
  }
  return result;
}
...
async function main(): Promise<void> {
  ...
  const kinds: readonly EffectKind[] = ["issue_refund", "create_return_label", "change_booking", "refuse_request"];
  ...
  const resolutions = Object.fromEntries(
    Object.entries(rawResolutions).filter(([, outcome]) => outcome !== "unknown"),
  ) as ServiceEnvironment["resolutions"];
