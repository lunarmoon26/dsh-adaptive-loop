...
No v0 command runs GEPA, SkillOpt, another optimizer, or an LLM — except `dal propose run`, which sends one fixed-route text-only OpenAI, Anthropic, or DeepSeek HTTPS request after verifying an exact approved, unexpired `send_data_externally` decision bound to the provider-versioned request digest prepared by `dal propose prepare --model <id> --budget <file>`. OpenAI and DeepSeek requests remain byte-for-byte v2; new Anthropic Sonnet 5 requests use v3 with exact `thinking: {type: "disabled"}`. V1/v2 historical requests remain unchanged, and an old Anthropic approval cannot authorize v3. A durable budget reservation precedes credential access and sending; reservations are allocations, not measured billing or DSH e2e spend enforcement. Payload-only approvals do not authorize new sends. The proposer loads no DSH profile, workspace tools, subprocess, or `.env` file; explicit launcher credential loading is separate. See [`docs/proposer-request.md`](docs/proposer-request.md). No v0 command changes dsh profiles or `$DSH_HOME`, except `dal install user-global --approval <decision-file>`, which writes only the `end-task-feedback` skill under `~/.agents/skills/` and the fixed user-global `AGENTS.md` under `$DSH_HOME`, and only after verifying an exact approved, unexpired decision at the operation. Keep other profile and optimizer behaviors future-only in [`ROADMAP.md`](ROADMAP.md).
