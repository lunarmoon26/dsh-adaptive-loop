...
No v0 command runs GEPA, SkillOpt, another optimizer, or an LLM — except `dal propose run`, which sends one fixed-route, text-only DeepSeek HTTPS request after verifying an exact approved, unexpired `send_data_externally` decision bound to the complete versioned request digest prepared by `dal propose prepare --model <id>`. Payload-only approvals do not authorize sending. The proposer loads no DSH profile, workspace tools, subprocess, or `.env` file; see [`docs/proposer-request.md`](docs/proposer-request.md). No v0 command changes dsh profiles or `$DSH_HOME`, except `dal install user-global --approval <decision-file>`, which writes only the `end-task-feedback` skill under `~/.agents/skills/` and the fixed user-global `AGENTS.md` under `$DSH_HOME`, and only after verifying an exact approved, unexpired decision at the operation. Keep other profile and optimizer behaviors future-only in [`ROADMAP.md`](ROADMAP.md).
